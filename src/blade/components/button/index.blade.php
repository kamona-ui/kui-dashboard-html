...
@php
    $baseClasses = 'inline-flex items-center justify-center transition-colors font-medium select-none disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-dark-eval-2';
    ...
    $classes .= ($leftIcon || $rightIcon) ? ' gap-2' : '';
...
@else
    <button
        {{ $attributes->merge(['type' => 'submit', 'class' => $classes, 'disabled' => $disabled]) }}
    >
        <x-button._body
            :iconOnly="$iconOnly"
            :srText="$srText"
